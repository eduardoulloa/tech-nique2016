			...
			<?php foreach($ejobs->job as $job){ ?>
			   <div id="job-listings">
			   		<ul id="recent-listing">
			   			<li>
			   				<h4><?php echo substr($job->job_title,0,40); ?></h4>		
							<?php if(trim($job->meta->job_shortdescription->value()) != ""){ ?>  												
							<p style="font-size:12px;"><?php echo substr($job->meta->job_shortdescription->value(),0,60)."..."; ?></p>
							<?php } ?>
			   				<p style="bottom:25px; position:absolute;"><?php esc_html_e($job->locationToString()); ?></br>
							...
							<?php endif; ?></i>
			   				...
			   				<img src="<?php bloginfo('template_directory'); ?>/img/arrow-2.png" alt="View">
			   			</li>			   			
			   		</ul>
